use crate::UnifierPair;
use crate::traits::{DatabaseEntry, Manifest};
...
/// Invalidates the cached entries for the records written by a transaction.
///
/// [`Database::commit`](crate::Database::commit) walks the transaction's buffer and calls
/// [`expire_record`](CacheExpiry::expire_record) for every record it is about to write, so a
/// transaction leaves the cache coherent with storage.
///
/// The cache struct generated by `manifest![Name + CacheType: ...]` implements this
/// automatically, dispatching over the manifest's record enum to the right
/// [`CacheContainer`]. [`NoCache`] implements it as a no-op for every manifest.
///
/// Implementing this by hand is only necessary for a cache that is not generated by the
/// `manifest!` macro.
pub trait CacheExpiry<M, U>
where
    M: Manifest<U>,
    U: UnifierPair + 'static,
{
    /// Expires the cached value stored under `record`'s primary key.
    fn expire_record<'a>(&mut self, record: M::Record<'a>)
    where
        U: 'a;
}

impl<M, U> CacheExpiry<M, U> for NoCache
where
    M: Manifest<U>,
    U: UnifierPair + 'static,
{
    fn expire_record<'a>(&mut self, _record: M::Record<'a>)
    where
        U: 'a,
    {
    }
