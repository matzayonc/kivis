...
//     Store::insert(&mut store, product1.clone()).unwrap();
//     assert_eq!(Store::get(&store, &key1).unwrap(), Some(product1.clone()));

//     // Insert product2 with same key should overwrite
//     Store::insert(&mut store, product2.clone()).unwrap();
//     assert_eq!(Store::get(&store, &key1).unwrap(), Some(product2));
// }
...
//     // Insert all orders
//     Store::insert(&mut store, order1.clone()).unwrap();
//     Store::insert(&mut store, order2.clone()).unwrap();
//     Store::insert(&mut store, order3.clone()).unwrap();

//     // Verify all can be retrieved with their respective keys
//     assert_eq!(Store::get(&store, &key1).unwrap(), Some(order1));
//     assert_eq!(Store::get(&store, &key2).unwrap(), Some(order2));
//     assert_eq!(Store::get(&store, &key3).unwrap(), Some(order3));

//     // Remove one and verify others remain
//     Store::<OrderRecord>::remove(&mut store, &key1).unwrap();
//     assert_eq!(Store::<OrderRecord>::get(&store, &key1).unwrap(), None);
//     assert_eq!(
//         Store::<OrderRecord>::get(&store, &key2).unwrap().is_some(),
//         true
//     );
//     assert_eq!(
//         Store::<OrderRecord>::get(&store, &key3).unwrap().is_some(),
//         true
//     );
// }
